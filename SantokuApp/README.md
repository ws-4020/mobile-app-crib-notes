...
- [Setting up the development environment - React Native](https://reactnative.dev/docs/environment-setup)を参考に開発環境を構築してください。
  ...
  - インストールするNode.jsのバージョンはLTSバージョンとしてください。
    - （Windowsのみ）Chocolateyでインストールする場合は、 `node-lts` パッケージをおすすめします。
  - 「Development OS」は、開発に利用しているOSを選択してください。
  - （macOSのみ）「Target OS」は、「iOS」「Android」の両方の手順を実施してください。
  - （macOSのみ）CocoaPodsのバージョン差異によって`pod install`に失敗することがあります。CocoaPodsは[Bundler](https://bundler.io/)を利用して、次の方法でインストールしてください。
  ...
  - （macOSのみ）[ios-deploy](https://github.com/ios-control/ios-deploy)を[Installation](https://github.com/ios-control/ios-deploy#installation)にしたがってインストールしてください。
    - `npm run ios -- --device='<device name>'`で、デバイスにアプリをインストールするために必要です。
