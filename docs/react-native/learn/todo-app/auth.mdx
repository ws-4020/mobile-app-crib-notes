...
まずは、ナビゲーション定義を修正します。修正イメージは以下のとおりです。
...
import React from 'react';
import {AuthedStackNav, UnauthedStackNav} from 'navigation';
...
以上でナビゲーション定義の修正が完了しました。

次ににログイン機能を追加します。今まではただ画面遷移させていた処理を認証処理に変更します。
...
最後に`UserSetting`画面を修正し、ログアウトできるようにします。

修正量が多いので、次のソースコードで`/src/screens/settings/UserSetting.tsx`を上書きしてください。
