...
ToDo一覧画面へ遷移できるようになりましたが、ただ画面を移動しているだけなので認証制御はできていません。
ユーザの認証状態に応じて画面を切替られるよう修正しましょう。
...
ログインボタンを押下すると、`userContext.login(userName, password)`を呼び出して認証状態にします。

最後に`TodoBoard`画面を修正し、ログアウトできるようにします。
