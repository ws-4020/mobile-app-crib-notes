...
構成の説明として、
ここまででとこれから作成する構成の違いがわかるよう色分けしています。
背景色または、実線が青色になっている部分がこれから構成する部分を表しています。
...
<Tabs
  ...
  values={[
    {label: 'エミュレータ/シミュレータ', value: 'emulator'},
    {label: '実機', value: 'machine'},
  ]}>
...
![Image](emulator-api-env.png)

</TabItem>
<TabItem value="machine">

![Image](machine-api-env.png)
...
[ハンズオン用 バックエンドサンプルアプリ](https://github.com/ws-4020/mobile-app-hands-on-backend#readme)にそれぞれの起動と確認方法が記載されているので確認して実行してください。
...
`${IP}`部分を確認したIPアドレスへ変更してアクセスしてください。
