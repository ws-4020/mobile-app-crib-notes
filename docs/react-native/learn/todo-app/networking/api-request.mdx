...
:::note
Windowsのコマンドプロンプトで上記コマンドを実行する場合は、マルチバイトの文字化けを防ぐため、送信するデータをファイルにて指定する必要があります。

以下の内容でUTF-8のファイルを作成し、`-d "{\"text\": \"アプリ外からの登録\"}"`のかわりに`-d@${作成したファイル名}`と指定してcurlコマンドを実行してください。
