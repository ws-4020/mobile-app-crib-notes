...
REST APIの呼びだすと応答を受け取るまでに少し時間がかかることがあります。呼びだし中に何もフィードバックを返さないと、アプリで処理中なのか、自分の操作が伝わっていないのか、ユーザには判断できなくなってしまいます。
...
REST APIを呼びだす`TodoService.getTodos()`の前でloading状態を設定し、`finally`で`loading`を解除します。
