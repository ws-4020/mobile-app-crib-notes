...
REST APIの呼びだしには少し時間がかかるものです。呼びだし中に何もフィードバックを返さないと、アプリで処理中なのか、自分の操作が伝わっていないのか、ユーザには判断できなくなってしまいます。
...
まず、REST APIを呼びだすコンポーネントで、REST APIを呼びだし中かどうかを、ReactのStateで管理します。

次のように `useState` を利用して`loading`状態を管理します。
REST APIを呼びだす`TodoService.getTodos()`の前でloading状態を設定し、`finally`でloadingを解除します。

```typescript diff title="screens/todo/TodoBoard.tsx"
...
次に、REST APIを呼びだし中にアクティビティインジケータを表示するように修正します。このとき、アクティビティインジケータを表示するコンポーネントをToDo一覧の上にかぶせて表示することで、ToDo一覧内でのユーザの操作をブロックします。
処理中（`true`）の時だけアクティビティインジケータを中央に表示し、操作できないようにToDo一覧を触れないように覆います。

ToDo一覧の操作は出来ませんが、設定タブをタップし、設定画面に遷移することは出来ます。
...
ReactNativeの `ActivityIndicator` を利用してローディングを表示します。
操作ブロック用のContainerがあるため、loading中でなければ表示しない制御も入れます。

```typescript diff title="/src/screens/todo/TodoBoard.tsx"
...
次の動作確認ではREST APIの応答を一定時間待つという状態を擬似的に作り出してみましょう。
:::
...
:::note
ここではREST APIの応答後に`await`を利用して`Promise`が`resolve`するまで1秒間だけ待つように実装しています。
