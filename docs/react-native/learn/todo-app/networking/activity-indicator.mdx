...
ここまでを実装した結果、以下の不具合が発生します。

- `ToDo一覧画面`でToDoのステータスを更新した後に、`未完了のみ`または、`完了のみ`へタブ移動した結果、表示されているToDoが意図して更新したものと一致しない。
- `ToDo登録画面`でToDoの`追加ボタン`を押した直後にヘッダーの`×ボタン`を押すと、ToDo登録APIの処理結果を待たずに`ToDo登録画面`を閉じてしまいます。
その後、`goBack()`で前画面に戻ろうとすると不整合（既に対象画面へ遷移しているのにまた遷移しようとしている）が起きて、エラーが発生。

これらの事象を解消するためにそれぞれの修正を実施します。

まずは、ToDoステータス更新の不具合から説明します。ToDoの更新時、ToDo更新APIの応答前に一覧画面を更新するよう`toggleTodoCompletion`へ追加します。
...
:::caution
現時点では、REST APIで発生したエラー処理は実装していません。本来は開発するアプリの仕様にあわせて例外処理（`catch`）を実装する必要があります。
:::

このように修正することで、ToDo更新APIを呼びだす前に更新内容を画面内へ反映されるので不具合が解消されます。
つづいて、ToDo登録APIの応答後に`ToDo登録画面`がフォーカスされているかを判定して、フォーカスされている場合、元画面へ戻るように修正します。
...
これで、`goBack()`前にフォーカスされている画面が判定されるようになり、不整合が発生しなくなります。

## 動作確認

これで、ToDo一覧取得のREST APIを呼びだしている間はアクティビティインジケータが表示され、一部の操作はブロックされるようになりました。
あわせて不具合箇所も解消されました。実際にアプリを起動して動きを確認しましょう。
...
- アクティビティインジケータの表示と一部操作がブロックされていることの確認。
- ToDo更新時、`ToDo一覧`の各ToDoのステータスが意図した状態で表示されていることの確認。
- ToDo登録時、`ToDo登録画面`にてToDo登録APIの処理が完了する前にヘッダーの`×ボタン`を押した際の不整合が解消されていることを確認。

まずは、アクティビティインジケータの表示と一部操作がブロックされていることの確認です。次の手順で確認してください。
...
次にToDo更新の確認を進めます。次の手順で確認してください。
...
4. ToDoの登録（4件）
5. 任意のToDoのステータスを更新
6. `未完了のみ`または、`完了のみ`のタブへ移動して、ToDoの確認

`5.`で更新したToDoがタブ内に表示されていることが確認できます。
...
3. ログイン
4. `＋ボタン`を押下
5. `ToDo登録`画面で、任意のToDoを入力して、`追加ボタン`を押下
6. `5.`の直後に、ヘッダーの`×ボタン`を押下
