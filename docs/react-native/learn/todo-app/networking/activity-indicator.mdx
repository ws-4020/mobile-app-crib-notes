...
処理中（`true`）の時はアクティビティインジケータを表示。それ以外の場合は非表示とします。
ユーザへ処理中の時は、操作出来ないことを伝える為、アクティビティインジケータを画面中央に表示します。
...
+          <ActivityIndicator animating={loading} color="red" style={styles.indicator} size="large" />
+        </View>
+      )}
    </View>
  );
...
今回は、REST APIを呼び出したら応答までの時間を設定する部分を汎用的に適用したいので、Middlewareとして切り出します。
...
`setTimeout(resolve, 1000)`の第二引数へ、待機時間を設定します。今回の例は`1000`（1秒）を設定します。

このように修正すると、`TodoService`でToDo一覧などを取得するまでに1秒かかるようになります。なので、その間アクティビティインジケータが表示され、1秒経つとインジケータが消えることを確認しましょう。
