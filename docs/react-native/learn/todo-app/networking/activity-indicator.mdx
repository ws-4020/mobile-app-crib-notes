...
今の状態で、`ToDo登録画面`でToDoの`追加ボタン`を押した直後にヘッダーの`×ボタン`を押すと、ToDo登録APIの処理結果を待たずに`ToDo登録画面`を閉じてしまいます。
その後、`goBack()`で前画面に戻ろうとすると不整合（既に対象画面へ遷移しているのにまた遷移しようとしている）が起きて、エラーが発生してしまいます。
この事象を解消するために以下の修正を実施します。

- ToDoの更新時、ToDo更新APIの応答前に一覧画面を更新するよう`toggleTodoCompletion`へ追加する。
- ToDo登録APIの応答後に`ToDo登録画面`がフォーカスされているかを判定する。フォーカスされている場合、元画面へ戻るように修正します。
