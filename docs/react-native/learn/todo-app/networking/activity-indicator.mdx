...
Todo一覧画面マウント時にTodo一覧取得APIの呼び出しをするので画面中央にアクティビティインジケーターが表示されることと操作がブロックされていることが確認できると思います。

:::note
REST API呼び出しの応答時間を1秒に設定しています。確認の際に1秒だと早すぎる場合は、お好みの時間に変更して再度確認してみてください。
:::
