...
ただし、設定タブから設定画面を表示することは出来るようにしておきます。
...
ToDo一覧画面マウント時にToDo一覧取得APIを呼び出していますが、その呼び出し中は画面中央にアクティビティインジケーターが表示さて、ToDo一覧に対する操作がブロックされていることが確認できます。
