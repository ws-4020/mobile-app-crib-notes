...
- `ToDo登録画面`でToDoの`追加`ボタンを押した直後にヘッダーの`×`ボタンを押すと、ToDo登録APIの処理結果を待たずに`ToDo登録画面`を閉じてしまいます。
...
- ToDo登録時、`ToDo登録画面`にてToDo登録APIの処理が完了する前にヘッダーの`×`ボタンを押した際の不整合が解消されていることを確認。
...
3. ログイン
4. `＋`ボタンを押下
5. `ToDo登録`画面で、任意のToDoを入力して、`追加`ボタンを押下
6. `5.`の直後に、ヘッダーの`×`ボタンを押下
