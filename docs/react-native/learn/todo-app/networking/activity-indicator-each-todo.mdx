...
次はToDoのチェックボックスをタップしてから、完了状態がUIに反映されるまでインジケータを表示し、ブロックしてみましょう。

ToDo一覧の最新化と同じように処理が終わるまで操作をブロックしても良いですが、連続してToDoの完了状態を変更するときに、かなりのストレスになります。

ここではチェックボックスをタップされたToDoだけを処理中のUIに変更し、操作をブロックしてみましょう。

下のイメージはアクティビティインジケータを表示している画面です。TODO画像張替え。
![Image](./indicator.png)
...
まずは`TodoBoard.tsx`から`TodoList.tsx`に処理中のToDoのIDのリスト（`processingTodos`）を渡します。
...
次に`TodoList`で処理中のToDoを判定し`TodoItem`の`loading`状態を設定します。

受け取るパラメータ（`Props`）に`loading`を追加してインジケータと、ToDoの操作ブロックするViewを表示できるようにしましょう。

`TodoList.tsx`でも`TodoItem`にloadingを渡すようにします。
...
+export const TodoItem: React.FC<Props> = ({id, text, completed, toggleTodoCompletion, onRemove, loading}) => {
```

## 処理中のUIを表示する

`TodoItem`に処理状態が渡せたので、処理中のUIを表示するように修正します。
アクティビティインジケータと操作ブロック用のViewを表示できるよう、次のように修正してください。
...
+          <ActivityIndicator animating={loading} size="large" color="white" style={styles.indicator} />
+        </View>
+      )}
     </View>
   );
