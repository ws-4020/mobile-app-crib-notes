...
次のように更新対象のToDoの状態を変更して`setTodos`でstateを更新した後にREST APIを呼びだすように修正します。

すでにUIは更新しているため、`todos`を更新する必要はないので`then`での処理も削除します。

`then`を削除する理由は、REST API呼び出し後にまたUIへ反映してしまい、完了状態が意図しない状態となってしまうためです。
