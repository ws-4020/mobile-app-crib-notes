...
`setTodos(todos.map((todo) => (todo.id === id ? returnedTodo : todo)))`では「複数のToDoを連続で更新する」ような操作をするとToDoの状態が戻ってしまうような事象が発生します。
これは「画面の実装」中には再現しづらい不具合ですが、「REST APIとの接続」した後でToDoのステータス更新処理で時間がかかるようになると発生します。
:::
