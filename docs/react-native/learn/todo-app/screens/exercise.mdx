...
ここでは関数型の更新を利用しない場合に「複数のToDoを連続してステータス更新する」と発生する不具合を紹介します。
...
REST APIに接続すれば不具合は再現できます。準備に時間がかかるため、ここではREST APIに接続した場合と同じ状況を再現します。
...
:::note
ヒント：不具合が発生するのはレンダリング用の`todos`をベースに更新していることが関係しています。
:::

### 処理完了後の画面遷移

ここではScreenの状態を判定しないと「登録処理待ちのときに登録画面を閉じる」ときに発生する不具合を紹介します。
...
正しくは`navigation.isFocused()`が`true`の場合のみ`navigation.goBack()`を呼びますが、
...
`TodoForm.tsx`と`TodoService.ts`を修正したらアプリを起動して、画面したのTabでHome、設定、Homeと遷移してください。
