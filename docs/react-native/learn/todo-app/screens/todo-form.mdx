...
:::note
`onAdd`では`navigation.isFocused()`が`true`の場合だけ`navigation.goBack()`しています。
これは`postTodo`の処理中にユーザが右上の閉じるボタン（Xボタン）で画面を閉じてしまったユーザが意図しない画面遷移を防ぐためです。
「画面の実装」を学習中には発生しづらい不具合ですが、「REST APIとの接続」で実装を追加すると登録処理で時間がかかるようになるため不具合が発生します。
:::
