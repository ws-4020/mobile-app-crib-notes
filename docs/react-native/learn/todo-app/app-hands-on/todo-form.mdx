...
:::info
`TodoStackNav`ではなく、なぜ`AuthedStackNav`ナビゲータにタスク登録画面（`TodoForm`）を追加するのかと疑問を感じたかもしれません。
理由としては、タスク登録画面を表示する際はタブバーを非表示にしたかったからです。
`TodoStackNav`ナビゲータにタスク登録画面を追加した場合、下部にタブバーが表示されてしまいます。
推奨される方法は、タブスタック外側のナビゲータで画面を定義することです。
...
タスク登録画面ではこれを利用して、変更途中のデータがあった場合に破棄確認ダイアログを表示します。
...
詳細は、次のReact Navigation公式ドキュメントを参照してください。
...
:::caution
現時点ではタスク登録画面で登録したタスクが一覧画面に表示されません。
次のセクションでその対応を実施します。
:::
