...
:::info
`TodoStackNav`ナビゲータではなく、なぜ`AuthedStackNav`ナビゲータにタスク登録画面（`TodoForm`）を追加するのかと疑問に感じたかもしれません。
理由としては、タスク登録画面を表示する際にタブバーを非表示にしたかったからです。
...
このイベントはユーザーが画面を離れるときに発生し、それを防ぐことができます。
これを利用して、フォームに入力があり（`formik.dirty`が`true`）フォームがサブミットされていない（`formik.isSubmitting`が`false）場合において、破棄確認ダイアログを表示する処理を追加しています。

ナビゲーションイベント、および戻るを防ぐ方法の詳細は、次のReact Navigation公式ドキュメントを参照してください。
