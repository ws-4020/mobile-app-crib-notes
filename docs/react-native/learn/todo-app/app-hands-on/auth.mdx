...
:::note
用意する部品は[Webベースのハンズオン](https://fintan-contents.github.io/spa-restapi-handson/)のものと同等です。
本ハンズオンでの部品の説明は省略します。
:::
...
このパターンは、React Routerなど他のルーティングライブラリで長きにわたり使用されており、「保護されたルート」として一般に知られています。
ToDoアプリでもこのパターンに従い、認証制御を組み込んでいきます。
...
ログアウトボタンを押下すると、`userContext.logout()`を呼び出して認証を破棄します。
