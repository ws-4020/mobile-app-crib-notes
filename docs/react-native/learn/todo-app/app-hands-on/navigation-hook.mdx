...
[useNavigationの型定義](../../basic-concepts/react-navigation-basics/modal#usenavigationの型定義)にあるとおり、`useNavigation`に型を指定することで、型安全にナビゲーション操作できます。
しかしながら、`useNavigation`の型定義は複雑です。各自で個別定義するのは避けて、ナビゲータ毎のカスタムフックを事前に用意したほうがいいでしょう。
