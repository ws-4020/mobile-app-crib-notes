...
ログイン画面とTodo一覧画面を追加します。中身は後で実装するため、空の画面を実装します。
...
追加した画面をナビゲーターの対象として追加します。`src/navigation/RootStackNav.tsx`を修正してください。
...
次に、Welcome画面からログイン画面に遷移できるようにします。[To Doアプリの仕様](./app-spec.md)のWelcome画面のように、ヘッダーにログインボタンを追加しましょう。
