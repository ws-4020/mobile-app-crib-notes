...
Xcodeの「Product Bundle Identifier」（`project.pbxproj`の`PRODUCT_BUNDLE_IDENTIFIER`）の設定値をConfigurationごとに設定しています。
...
Xcodeでビルドタイプごとのアイコンアセットを用意し、Xcodeの「Asset Catalog App Icon Set Name」（`project.pbxproj`の`ASSETCATALOG_COMPILER_APPICON_NAME`）でビルドタイプごと異なるアイコンアセットを設定しています。
...
コード署名には、基本的には開発用プロビジョニングプロファイルを利用するように設定しています。配布用にIPAファイルを作成する場合は、App Store ConnectやDeployGateにアップロードする前に、XcodeのOrganizerなどから配布用プロビジョニングプロファイルで署名し直してください。
...
- Debug ConfigurationとDebugAdvanced Configurationで「Info.plist File」の設定項目（`project.pbxproj`の`INFOPLIST_FILE`）に`Debug.plist`を設定
