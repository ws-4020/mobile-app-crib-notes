...
プロダクトフレーバーをProductionとし、ビルドタイプをReleaseとするには、次のようにします。

```bash
npm run android -- --variant "productionRelease" --appIdSuffix ""
```

:::note
AndroidのアプリケーションIDとパッケージ名が異なるので、`appId`オプションを渡しています。`appId`を渡さない場合、次のようなエラーが発生します。
