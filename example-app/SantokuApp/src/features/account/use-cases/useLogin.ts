...
import {useLogin as useLoginService} from '../services/useLogin';
import {useTerms} from '../services/useTerms';
import {LoginForm} from '../types/LoginForm';

export const useLogin = (form: FormikProps<LoginForm>) => {
  const {termsOfService} = useTerms();
  const {mutateAsync: callLogin, isLoading} = useLoginService();
