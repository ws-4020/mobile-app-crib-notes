...
| レイヤー名 | 概要 |
|:--|:--|
| apps | アプリのトップレベルのレイヤーです。このレイヤーには後述する機能モジュールの頂点となる機能モジュールを配置します。通常ここには`app`という1つの機能モジュールのみが存在します。1つのコードで複数アプリに対応する場合、このレイヤーに複数の機能モジュールを配置します。ここに配置された機能モジュールは、アプリ起動時の初期化処理やコンテキスト（プロバイダー）、テーマ機能、画面やナビゲーションなどアプリ全般をコントロールします。 |
...
| hooks | form | ユーザが入力した値の保持やバリデーション定義などを実施します。 |
