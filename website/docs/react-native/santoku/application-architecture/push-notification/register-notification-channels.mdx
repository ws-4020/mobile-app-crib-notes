...
Android 8.0（API レベル 26）以降では、プッシュ通知はすべて通知チャンネルに割り当てる必要があります。通知チャンネルごとに、表示や鳴動動作などを適用します。
...
通知チャンネルには重要度を設定できます。（Android 7.1以前では、通知チャンネルが存在しません。その代わりに、通知の優先度を設定します。）
