...
このアプリでは、モバイル端末の紛失や盗難リスクに対応するため、PINコードやユーザの生体属性を使用して、ユーザが端末の所有者であることを確認する端末認証をアプリ起動時に実施します。
...
どの生体属性が登録されているか（例えば、指紋データが登録されているか）を確認することもできます。
...
:::caution
Androidでは、`disableDeviceFallback`をtrueにして、生体認証失敗時などのフォールバックを無効にした場合、`cancelLabel`も設定しないと以下のようなエラーが発生します。
