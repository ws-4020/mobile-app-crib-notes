...
Universal Linksの設定に従ったURLをユーザに連携することで、ユーザはそのURLからアプリを起動できます。

しかし、連携するURLには認証後にしか参照できないような情報を含めないように注意する必要があります。認証後に参照可能な情報は、URLタップ後に起動したアプリ内で必ず取得するようにします。
