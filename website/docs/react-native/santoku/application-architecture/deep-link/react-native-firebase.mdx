...
Firebase Dynamic Linksは、iOSで初回起動時にディープリンクURLを取得するために[クリップボードを利用するとドキュメントに記載されています](https://firebase.google.com/docs/dynamic-links/operating-system-integrations)。ただし、実際はクリップボードの値だけでなく、[デバイス情報なども利用](../../decisions/adr-004-deep-link.mdx#iosでの意図しない共有)しています。
