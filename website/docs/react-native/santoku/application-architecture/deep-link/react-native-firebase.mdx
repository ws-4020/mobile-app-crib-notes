...
Firebase Dynamic Linksは、iOSで初回起動時にコンテンツURLを取得するためにクリップボードを利用しますが、実際はクリップボードの値だけでなく、デバイス情報なども利用しています。

このため、例えばクリップボードが空だったとしてもコンテンツURLを取得できる場合があります。こういった場合は、ユーザ自身が実際に開こうとしたわけではないコンテンツURLが取得されている可能性があります。

コンテンツURLをユーザが開いたものかどうかを判定するために[matchType](https://firebase.google.com/docs/reference/swift/firebasedynamiclinks/api/reference/Enums/DLMatchType.html)という項目があります。
...
React Native LinkingではコンテンツリンクURLではなくディープリンクURLが取得されるため、[resolveLink](https://rnfirebase.io/reference/dynamic-links#resolveLink)を利用してコンテンツリンクURLを取得します。
