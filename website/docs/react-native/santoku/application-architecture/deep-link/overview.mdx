...
|URLの種類|説明|
|:-------|:--|
|ディープリンクURL|ユーザが開くURLで、OSがドメインとの関連を検証するときに利用するURLです。OSの検証が成功するとアプリケーションに渡されます。パラメータ部分にコンテンツリンクURLやフォールバックURLなどを保持します。|
|コンテンツリンクURL|アプリで表示したいコンテンツをあらわすURLです。アプリと関連付けていないドメインのURLも渡すことができます。|
...
破線以外の部分はDynamic Linksの機能ではなくOSが提供する[Universal Link（iOS）]()や[App Links（Android）]()の機能です。まずはこのOSの機能について説明します。
...
（図の④で）アプリケーションはOSからディープリンクURLを受け取ったあと、Firebase Dynamic LinksのSDKを利用してコンテンツリンクURLを取得します。
...
React Native Linkingを利用してディープリンクURLを受け取ることはできますが、ディープリンクURLからコンテンツリンクURLを取得する機能がありません。また、未インストール時のフローに対応できません。
...
アプリケーションがインストールされていない場合、ディープリンクのURLを開くとブラウザでURLが開かれます。このとき、ディープリンクのURLに対応するWebサイトが存在していないと、ユーザはコンテンツへの導線を失ってしまいます。インストールしていないユーザの導線を確保するために、ディープリンクを利用する場合はWebサイトも準備することが望ましいです。

本プロジェクトでは、WebサイトとしてDynamic Linksがデフォルトで提供する[アプリのプレビューページ](https://firebase.google.com/docs/dynamic-links/link-previews#app_preview_pages)を利用します。プレビューページではURLに対応するコンテンツ自体は表示できませんが、ストアなどに誘導できるため、ユーザが導線を失うことにはなりません。

### 環境とビルドの切り替え

テスト時（DevSantokuApp）では本番（SantokuApp）と異なるビルド設定でアプリをビルドします。
...
ディープリンクを利用する場合、ディープリンクURLとドメインの関連を検証するために証明書の情報や署名鍵の情報をFirebaseに登録する必要があります。
 ...
 - Firebaseアプリ毎に署名情報を設定します。
   ...
   - Androidは署名鍵のSHA256を複数アプリに設定できるため、ビルドで利用する署名鍵のSHA値をアプリに登録します。
