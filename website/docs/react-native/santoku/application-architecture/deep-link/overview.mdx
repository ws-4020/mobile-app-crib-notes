...
アプリケーション内で入力するより、URLを開くだけで操作できる方がUXとして好ましい場合にディープリンクを利用します。

ディープリンクの実現方式として[Firebase Dynamic Links](https://firebase.google.com/docs/dynamic-links)を利用し、
SantokuApp（React Nativeアプリ）でFirebase Dynamic Linksを利用する方式を記載します。
