...
ディープリンクから起動されたとき、アプリではそのURLを取得することができます。

例えば、リンクを開いたときにURLに応じた画面を表示するようにしたり、URLのパラメータから受け取った値を初期値として入力欄に設定しておくなど、幅広く活用することができます。これらの場合にディープリンクを利用しない場合、ユーザはアプリを起動してから手順に従って指定の画面に移動する必要があるなど、使いづらいアプリとなってしまうことがあります。
...
|URLの種類|説明|
|:-------|:--|
|ディープリンクURL|ユーザが開くURLで、OSがドメインとの関連を検証するときに利用するURLです。OSの検証が成功するとアプリケーションに渡されます。パラメータ部分にコンテンツリンクなどを保持します。|
|コンテンツリンク|アプリで表示したいコンテンツをあらわすURLです。アプリと関連付けていないドメインのURLも渡すことができます。|
...
本プロジェクトでは、WebサイトとしてDynamic Linksがデフォルトで提供する[アプリのプレビューページ](https://firebase.google.com/docs/dynamic-links/link-previews#app_preview_pages)を利用します。プレビューページではURLに対応するコンテンツ自体を表示することはできませんが、ストアなどに誘導することができるため、ユーザが導線を失うことにはなりません。
...
ディープリンクを利用する場合、ディープリンクURLとドメインの関連を検証するために事前に証明書の情報や署名鍵の情報をFirebaseに登録する必要があります。
