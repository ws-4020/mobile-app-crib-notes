...
Firebase Dynamic Linksの拡張機能では、未インストールのユーザがディープリンクURLを開いて[インストールしたあとに、そのままディープリンクURL](https://firebase.google.com/docs/dynamic-links/operating-system-integrations)を利用できます。
...
SantokuAppでは2つのFirebaseプロジェクト（DevSantokuAppとSantokuApp）を使用するため、Firebaseプロジェクトに応じた環境を構築します。

1. Firebaseプロジェクト毎にディープリンクURLで利用するURLプレフィックス（ドメイン）を[Fireabse Console上](https://console.firebase.google.com/u/4/project/_/durablelinks/links)で追加します。
1. [ビルドバリアント](../../development/build-configuration.mdx)にそってFirebaseアプリを作成します。
    - AndroidはアプリケーションID毎にFirebaseアプリを作成してください。
    - iOSはBundle Identifierとライセンスの組み合わせ毎にFirebaseアプリを作成してください。
1. FirebaseアプリにOSが検証するための設定値を追加します。
    - iOSではADPまたはADEPライセンスのTeamIDを設定します。
    - Androidは署名鍵のSHA256を設定します。これは複数設定できます。
1. 作成したFirebaseアプリケーション毎にFirebase Console上でディープリンクURLを作成し、アプリケーションを指定します。
1. ビルド設定にあったFirebaseアプリの設定ファイル（google-service.json、GoogleService-Info.plist）を取得してモバイルアプリプロジェクトに設定します。
1. モバイルアプリケーションの設定でドメインと関連付ける設定をします。
    - iOSではXcode上でCapabilitiesからAssociated Domainsに追加して[ドメインを関連付けます](https://firebase.google.com/docs/dynamic-links/ios/receive?hl=ja#open-dynamic-links-in-your-app)。
    - Androidではアプリマニフェストに[設定を追加してドメインを関連付けます](https://developer.android.com/training/app-links/verify-site-associations?hl=ja#request-verify)。
