...
|ウェブサイト|ディープリンクURLを開いたときにアプリケーションがインストールされていないときに開くサイト。|
...
本プロジェクトでは、WebサイトとしてDynamic Linksがデフォルトで提供するウェブサイトと[アプリのプレビューページ](https://firebase.google.com/docs/dynamic-links/link-previews#app_preview_pages)を利用します。ウェブサイトやプレビューページではURLに対応するコンテンツ自体は表示できませんが、ストアなどに誘導できるため、ユーザが導線を失うことにはなりません。
