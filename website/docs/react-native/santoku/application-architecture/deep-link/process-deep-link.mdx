...
コンテンツリンクURLをアプリケーションが取得したとき、（認証が必要など）すぐに処理するページに遷移できないケースがあります。複数の画面にまたがって保持する時には、コンテクストに設定するか画面間で持ち回る必要があります。

コンテンツリンクURLをコンテクストに設定してしまうとクリアするタイミングなどの設計が難しく、管理が煩雑になります。そのため、コンテンツリンクURLを引き継ぐ範囲を決めてナビゲーションのパラメータとして画面間で持ち回るように設計してください。
...
iOSのみですが、Firebase Dynamic Linksは、アプリがまだインストールされていないときに開いたディープリンクURLをアプリから取得するために、次のようにクリップボードを利用しています。
* ブラウザで表示したプレビューページからフォールバックURLに遷移する際に、ディープリンクURLをクリップボードにコピー
