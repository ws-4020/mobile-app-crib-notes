...
Androidのウォームスタートの場合、Expo Linkingの[addEventListener](https://docs.expo.dev/versions/latest/sdk/linking/#linkingaddeventlistenertype-handler)に設定した処理が呼ばれます。
この中でURLに応じた処理（画面遷移など）を行います。
...
このアプリでは、`android:launchMode`に`singleTask`を設定しているため、ウォームスタート時にActivityのインスタンスが作成されません。この場合、ディープリンクのURLはExpo Linkingの`addEventListener`で取得できます。
