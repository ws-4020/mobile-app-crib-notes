---
title: ログ出力
---

ログ出力は、以下のクラスで構成されます。

- Logger ... ログを出力するAPIを提供するクラスです。他2つのクラスを指定できます。
- LogFormatter ... メッセージを出力する際のフォーマットを表すクラスです。
- Transport ... ログの出力先を表すクラスです。出力先はコンソールやFirebase Crashlyticsがあります。

ログ出力で提供される機能は以下のようになります。

## ログレベルによるログの振り分け

以下のログのレベルを指定して出力を振り分けることができます。

- Trace
- Debug
- Info
- Warn
- Error

ログレベルには上記の箇条書きの記載通りに順序があり、ロガーに設定したログレベルより、同じか下のレベルがログとして出力されます。
例えば、ロガーのログレベルにTraceを指定すると、すべてのログレベルが出力されます。
ログレベルにWarnを指定すると、WarnとErrorのみが出力されます。

エラー出力にはエラーコードの指定を必須としています。エラーの問題究明をする際にこのエラーコードがキーワードになります。

## ログ出力先の指定

ログの出力処理は、トランスポートで行われます。  
ロガーの初期化で複数のトランスポートを指定することで複数の出力先に出力できます。  

```typescript
  const formatter = new SimpleLogFormatter();
  const consoleTransport = new ConsoleTransport();
  const firebaseCrashlyticsTransport = new FirebaseCrashlyticsTransport();
  // 初期化時に、コンソール出力とFirebase Crashlytics出力するトランスポートを渡す例
  const log = new Logger({level: 'trace', formatter, transports: [consoleTransport, firebaseCrashlyticsTransport]});
```

### Firebase Crashlyticsについて

モバイルアプリがクラッシュするのは、ユーザ離脱率の悪化につながります。
クラッシュ発生前のエラー情報を収集するためのトランスポートがFirebaseCrashlyticsTransportです。
クラッシュ発生後の次の起動時にエラーのログ出力情報をFirebase Crashlyticsに送信します。

Firebase Crashlyticsの設定については、[Firebase Crashlyticsの設定](../error-handling/how-to-handle-error#firebase-crashlyticsの設定)を参照してください。

## ログのフォーマット

ロガーはフォーマッタを関連づけることができます。
フォーマッタでは、出力する内容をラベルをつけたり日時をつけたり加工できます。
SimpleLogFormatterでは以下の内容が出力されます。

```typescript フォーマット出力例
  // Error以外の場合
  //   [＜ログレベル＞] ＜ログのメッセージ内容＞
  log.info("ロード処理完了")
  //  -> "[info] ロード処理完了"

  // Errorの場合
  //   [＜ログレベル＞] [＜指定したエラーコード＞] ＜ログのメッセージ内容＞
  try {
    /* 中略 */
  } catch {
    log.error("xxx処理例外", "E001XXX")
    //  -> "[error] [E001XXX] xxx処理例外"
    /* 中略 */
  }
```
