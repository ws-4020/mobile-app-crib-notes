...
onRefreshでReact Queryのrefetch関数を呼び出すことで、データを再取得します。
...
一方で、isFetchingはキャッシュの有無に関係なくクエリ実行中にtrueとなります。
...
条件が複雑になる場合や、複数のAPIをループで呼び出す場合などは、非同期関数に纏めたほうが分かりやすいです。
...
FlatListのonEndReachedでReact QueryのfetchNextPage関数を呼び出すことで、リストの末尾にスクロールした際にデータを追加取得します。
データの追加取得中は、ListFooterComponentに指定したインジケータが表示されます。
FlatListのonRefreshでReact Queryのrefetch関数を呼び出すことで、Pull To Refresh操作でデータを再取得ができます。
