...
指定できるオプションや戻り値は、TanStack Queryの[useQuery](https://react-query.tanstack.com/reference/useQuery)とほぼ同様です。
利用時のコード例は以下のようになります。
...
第一引数にはクエリパラメータ、第二引数にはオプションを設定できます。
指定できるオプションや戻り値は、TanStack Queryの[useInfiniteQuery](https://react-query.tanstack.com/reference/useInfiniteQuery)とほぼ同様です。
利用時のコード例は以下のようになります。
...
基本的な使い方や指定できるオプションは、TanStack Queryの[useMutation](https://react-query.tanstack.com/reference/useMutation)とほぼ同様です。
