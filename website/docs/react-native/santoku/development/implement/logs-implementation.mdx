...
Firebase Crashlyticsコンソールで送信されたメッセージやスタックトレースが確認できます。
しかし、スタックトレースに関してはソースコードのマッピングがないため、直感的にはわからない表示になります。  
...
以下の手順を行うことでソースコードとのマッピングを解決して表示できます。
...
ここでは、［プロジェクトルート］/stack-trace.txtに保存する仮定とします。
