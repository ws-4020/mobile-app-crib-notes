...
なお、PrettierやEditorConfig、ESLintの[fix](https://eslint.org/docs/latest/use/command-line-interface#--fix)オプションで修正可能なものについては、コードのスタイルなど機械的に修正可能なものです。チェック対象外にする正当な理由は考え難いため、必ず修正するようにしてください。
...
```typescript
new Person(); // eslint-disable-line no-new -- コンストラクタの検証のみをテストするため、作成したインスタンスを使用しない

// eslint-disable-next-line no-new -- コンストラクタの検証のみをテストするため、作成したインスタンスを使用しない
new Person();

/*
  eslint-disable-next-line no-new
  コンストラクタの検証のみをテストするため、作成したインスタンスを使用しない
 */
new Person();
```
