...
### 型エイリアス名は基本的には名詞としてください

型エイリアス名は、基本的にその型エイリアスの特徴を端的に表す名詞とし、先頭の文字は大文字で開始してください。

### コンポーネント名は基本的には名詞としてください

コンポーネント名は、基本的にそのコンポーネントの特徴を端的に表す名詞とし、先頭の文字は大文字で開始してください。
...
ただし、`for`文等のループ構文において、カウンタとして使用される変数の名称には、 慣用的に使用される変数名（`i, j, k ...`）を使用してもよいです。

また、一時的に使用されるスコープの狭い変数は意味の無い名前でも構いません。
...
class ItemList {
  ...
  public getTotalPrice() {
    return this.items.map(item => item.price).reduce((price1, price2) => price1 + price2);
  }
}
```
...
function findUnreadNotifications() {
  ...
  for (const notification of notifications) {
    const readNotification = {...notification, status: Status.ALREADY_READ};
    service.update(readNotification);
  }
...
### 配列やオブジェクト全体をコピーする場合はスプレット構文を使用してください

配列やオブジェクト全体をコピーする場合はスプレット構文を使用するのが最もシンプルです。
