...
具体的な設定方法として、ビルドタイプはAndroidではGradle(`buildTypes`)、iOSではXcodeのConfigurationを利用し、プロダクトフレーバーは[react-native-config](https://github.com/luggit/react-native-config)を利用しています。
