...
Config Pluginsは特定の規則に従った順序で実行されますが、一部の条件下においては順序に規則性がありません。また、現状の規則が今後のExpoアップグレードにによって変更が入る可能性もあります。
そのため、基本的には順序に影響されないConfig Pluginsを作成することが望ましいです。

ただし、場合によっては順序を意識する必要があるプラグインもあるかと思います。そのため、ここではConfig Pluginsが実行される順序について説明します。
...
Prebuild実行時、Config PluginsはModifierの単位で実行されます。実行順序は以下となっています。
