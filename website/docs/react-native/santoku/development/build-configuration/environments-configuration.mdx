...
そのため、環境の切り替えを実施するコマンドについては、[環境やビルドタイプに応じたアプリ起動](./app-launch-per-build-settings)を参照してください。
:::

Expo configでは、環境変数`ENVIRONMENT`の値によって使用する設定を切り替えます。

> TODO: コメントを追記する。特に、defaultAppConfigと環境毎のConfigについて
> もっと省略しちゃってもいいんじゃないかん
> ソースへのリンクを追加
