...
前述したように、`Prebuild`時に生成するネイティブプロジェクトの設定は、Expo configに指定した値で変更することが可能です。
Expo configに設定された値は、Expoが標準で用意しているConfig Pluginsによって、`Prebuild`時に生成されるネイティブプロジェクトに反映されます。
