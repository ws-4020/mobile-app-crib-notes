...
単体テストでは、主に開発した画面や機能単位のレイアウト（表示崩れなど）と動作を検証します。これにより、画面や機能単位の品質を保証することが目的となります。詳細は[全体テスト計画書のテスト工程定義](/react-native/santoku/test-planning/test-all-planning.mdx#テスト工程定義)を参照してください。
