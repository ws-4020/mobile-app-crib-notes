...
:::note info
今回はReactの[テストドキュメントで推奨されているツール](https://reactjs.org/docs/testing.html#tools)を選択しました。そのため、他のテスティングフレームワークおよびテスト用ライブラリとの比較検討を実施していません。
なお、Reactのテストドキュメントについては[こちら](https://reactjs.org/docs/testing.html)を参照してください。
