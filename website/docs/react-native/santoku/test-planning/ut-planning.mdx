...
- Androidエミュレータ／iOSシミュレータ
  ...
  [同一構成のエミュレータの利用](/react-native/santoku/development/test/emulator-with-same-config.mdx)を参照し、エミュレータ／シミュレータを準備します。OSバージョン・アスペクト比率・解像度などの構成については、上記のテスト端末に合わせてください。
