...
上記コマンドを実行すると、カバレッジレポートファイルが`coverage`フォルダに出力されます。もし、`c0/c1`のカバレッジが100%に達していない場合は、下記のフローに従ってください。
...
- テスト端末
  ...
  [テスト端末バリエーション](/react-native/santoku/test-planning/test-variation.mdx)から選定します。単体テストで使用するテスト端末は、AndroidとiOSのサポート対象内の最新OSを搭載した次の端末です。単体テストにおいて、Androidスマートフォンの実機を使う予定はありませんが、後述のエミュレータ利用にて同様の構成で作成するため、記載しています。
