...
:::note info
テスティングフレームワークとテスト用ライブラリは以下の理由により複数候補を立てて比較検討を行っていません。

- JestはMeta（旧：Facebook）が提供するJavaScriptテスティングフレームワークであり、ReactのテストリファレンスにおいてはJestが前提となっている。またMeta（旧：Facebook）自身も使用している。
- Reactのテストリファレンスにおいて、React Native Testing Libraryの使用が推奨されている。
:::
...
:::note info
モックサーバは複数候補を立てて、比較検討を行っていません。
これはモックサーバ自体年々新しいものが増えてきているため、比較対照するよりもPJ毎に最適なモックサーバを選択する方が良いと判断したためです。
今回選定したMockoonはstubby4nodeやSwaggerなど他のモックサーバに比べ、以下の点において機能が充実していたため選択しました。

- 既存のJSONファイルが使えたり、またホットリロードに対応しているためテストデータが作りやすい
- 条件によってレスポンスを変えられるため、エラーケースやバックエンド処理の結果で分岐する機能をアプリだけでテストしやすい
- レスポンスが送信されるタイミングを変える事ができるため、レスポンス遅延時のアプリ動作確認などができる
:::
