...
AndroidではshowBuildingsの設定値（true/false）に関わらず確認したバージョン全てで建物の輪郭が表示されました。
iOSでは16.2で指定値に関わらず常に建物の輪郭を表示し、それ以前の15.4と14.5では表示・非表示を選択することができました。

以下にエミュレータで検証した結果を示します。
