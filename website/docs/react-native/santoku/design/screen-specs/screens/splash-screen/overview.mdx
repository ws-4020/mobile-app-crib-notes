...
| イベント | 処理概要 |
|:------|:------|
| 初期表示 | 初期化処理を実施します。<br /> <li>[ログイン済みアカウントの利用規約の同意状態確認API][]を呼び出し、有効な利用規約に同意済みか確認します。同意済みであればホーム画面へ遷移、未同意であれば利用規約同意の画面へ遷移します。</li> |
...
<!-- link定義を使う場合は以下に記載する -->
[ログイン済みアカウントの利用規約の同意状態確認API]: /react-native/santoku/design/api-document#operation/get-accounts-me-terms
