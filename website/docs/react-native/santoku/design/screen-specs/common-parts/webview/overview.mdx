...
- エラーハンドリングは、[Application Architecture](/react-native/santoku/application-architecture/error-handling/overview.mdx)に準拠します。
  - onError/onHttpError発生時に表示するコンポーネントや、処理を親画面から指定できます。
...
※ onErrorとonHttpError時の画面は、親画面から変更可能です。変更がある場合は、個別の画面設計書に記載します。
...
| O2  | エラーメッセージ | Snackbar  | -[^2]                | コンテンツ取得でonErrorが呼び出された、かつ親画面から非表示にしていない場合のみ表示 |
...
|:----|:------------|:-----------|:----------|:----------------|-------------------------------------------|
| T1  | 取得したコンテンツ | WebView    | スクロール | 最底部に到達[^3]  |                                           |
[^3]コンテンツを一番下までスクロールした場合のみ発生。
