...
- 最底部までスクロールした時の処理を、親画面でハンドリングできます。
...
|:----|:--------------|:-----------|:--------------------|:--------------------------------------------|
| O1  | 取得したコンテンツ   | WebView    | 外部（※1）          | コンテンツ取得に成功した場合のみ表示             |
| O2  | エラーメッセージ | Snackbar  | メッセージキー：`app.webview.onError`（※2） | コンテンツ取得でonErrorが呼び出された、かつ親画面からonError発生時の処理が指定されていない場合のみ表示 |
