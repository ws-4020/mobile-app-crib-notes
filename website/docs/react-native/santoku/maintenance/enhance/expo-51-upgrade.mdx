...
アプリのコードで該当のAPIを利用している場合もしくはAppleの指定するSDKが利用されている場合、それらの利用が承認される理由を[プライバシーマニフェストファイル](https://developer.apple.com/documentation/bundleresources/privacy_manifest_files)に記載する必要があります。
