...
これらのリソースはアプリ独自に作成したConfig Pluginで利用します。
...
||SplashScreenForeground.imageset/splashscreen_foreground&#64;1x.png|
||SplashScreenForeground.imageset/splashscreen_foreground&#64;2x.png|
||SplashScreenForeground.imageset/splashscreen_foreground&#64;3x.png|
...
`config/plugin/src`フォルダにアプリ独自のConfig Pluginを作成しました。
...
prebuildの際に`expo`、`react-native`のバージョンが更新され、`expo-status-bar`への依存性が追加されました。
prebuildの際の`dependencies`フィールドの更新については、[Expoの公式記事](https://docs.expo.dev/workflow/prebuild/#dependencies)を参照してください。

#### `expo-build-properties`

アプリ独自のConfig Pluginで使用するため、`expo-build-properties`を依存ライブラリに追加しました。

#### `react-native-config`

react-native-configを使用しないことにしたので、依存ライブラリから削除しました。
