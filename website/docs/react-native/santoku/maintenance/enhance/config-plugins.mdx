...
このPull RequestはExpo SDK 47で取り込まれており、Expo SDK 47以降において、プロキシ環境下で`Prebuild`が動作することは確認しております。
しかし、このアプリのExpo SDK対応バージョンは46のため、このアプリにおいては、現時点での動作は検証出来ておりません。
プロキシ環境下で開発を進めるプロジェクトにおいては、Expo SDK 47へのアップグレード対応と合わせてConfig Pluginsに対応することをお勧めします。
:::
