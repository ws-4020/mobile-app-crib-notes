...
1. [テストコードの修正](#テストコードの修正)
   ...
   - [`jest-mock`における`expo-constants`のdeprecated警告への対応](#jest-mockにおけるexpo-constantsのdeprecated警告への対応)
...
#### `jest-mock`における`expo-constants`のdeprecated警告への対応
...
- [Prebuild](../../development/build-configuration.mdx#prebuild)時に生成・更新されないファイル
  - `android/`, `ios/`以外のファイル（`.gitignore`など）の変更を取り込む必要のある場合があります
