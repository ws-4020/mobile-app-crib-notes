...
1. [test修正](#test修正)
   ...
   - [`react-native` Dimensionsのmock失敗対応](#react-native-dimensionsのmock失敗対応)
   - [`jest.advanceTimersByTime`を`act`でwrap](#jestadvancetimersbytimeをactでwrap)
   - [`jest-mock`における`expo-constants`のdeprecated warn対応](#jest-mockにおけるexpo-constantsのdeprecated-warn対応)
   - [アニメーション中のAnimatedStyleの僅かな変化に対応](#アニメーション中のanimatedstyleの僅かな変化に対応)
   - [slideOutDurationテストの時間指定が厳密すぎるとテストが失敗する問題修正](#slideoutdurationテストの時間指定が厳密すぎるとテストが失敗する問題修正)
   - [keyExtractorの呼び出しが増えて失敗するようになったテスト修正](#keyextractorの呼び出しが増えて失敗するようになったテスト修正)
   - snapshot更新
1. [warn対応](#warn対応)
...
#### `react-native` Dimensionsのmock失敗対応
...
#### `jest-mock`における`expo-constants`のdeprecated warn対応

[Expo SDK 49の主な変更 - Expoがサポートするライブラリや機能 - Expo Constants](#expo-constants)に記載した通り、Constants.manifestが非推奨になりました。
mockする際に[スプレッド構文](https://developer.mozilla.org/ja/docs/Web/JavaScript/Reference/Operators/Spread_syntax)を使用してアクセスすると`warn`ログが出力されるようになっています。
...
#### slideOutDurationテストの時間指定が厳密すぎるとテストが失敗する問題修正
...
#### keyExtractorの呼び出しが増えて失敗するようになったテスト修正
