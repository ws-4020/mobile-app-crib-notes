...
Reactのバージョンは変更されておらず、依然として18.2.0です。

非推奨となっていた`ProgressBarAndroid`コンポーネントと`ProgressViewIOS`コンポーネントは、React Native 0.72.0で削除されたことに注意してください。

### デバッグ機能の改善

#### ネットワーク デバッグが JS デバッガーで利用できるようになりました

このデバッガーには、アプリがexpo-dev-clientまたはExpo Goで実行されるときのネットワーク リクエストのサポートも含まれるようになりました。

#### React devtools が Expo CLI に組み込まれるようになりました

shift+mを押して「Start React devtools」を選択することで起動できます。

#### Expo Tools 拡張機能を使用して VS Code でデバッグする
...
- [Expo Router v2.0](https://docs.expo.dev/routing/introduction/)
...
- [sentry-expo v7.0](https://docs.expo.dev/guides/using-sentry/)
  - クラッシュ レポート プラットフォームSentryと統合するライブラリ
  - 参考：[Migration for React Native | Sentry Documentation](https://docs.sentry.io/platforms/react-native/migration/#from-4x-to-5x)

#### Expo SDK モジュールのさまざまな改善

例えば、以下の改善があります。

- `expo-screen-orientation`は、長年にわたる多くの問題に対処するため大部分が再構築されました
- `expo-blur`がAndroidをサポートするようになりました
  - :warning: これは、特にローエンド デバイスでパフォーマンスに大きな影響を与える可能性があります
- `expo-clipboard`は`UIPasteControl`ボタンをエクスポートするようになりました

改善点の完全なリストは、[expo/CHANGELOG.md](https://github.com/expo/expo/blob/main/CHANGELOG.md#4900--2023-06-27)を参照してください。
...
#### Expo GoのAppleシリコン上でのネイティブ実行

iOSシミュレータのExpo GoがAppleシリコン上でネイティブ実行できるようになりました。
...
expo-dev-clientパッケージがプロジェクトにインストールされている場合、デフォルトで開発ビルドをターゲットにします。
...
package.jsonの`main`を変更して、任意のソースファイルを指すようにできます。
...
新しいアプローチは、プロセスをExpo Metro構成に統合し、JavaScriptエコシステムで一般的な規則に従って環境変数名の接頭辞に`PUBLIC`を含めることで、これらの問題を解決します。
...
package.jsonの`expo.install.exclude`のキーがサポートされるようになり、`expo start`などで行われる検証から除外するパッケージを指定できるようになります。
...
新しいアーキテクチャをまだサポートしていない唯一のモジュールはexpo-updatesです。
...
Reanimated API v1のサポートが削除されました。
v2とは互換性があります。

詳細は、以下のリンク先を参照してください。

- [Announcing Reanimated 3. Reanimated 3 release candidate is out… | by Krzysztof Magiera | Software Mansion](https://blog.swmansion.com/announcing-reanimated-3-16167428c5f7)

#### Expo Goでの`expo-face-detector`のサポート削除

この変更は、[Expo GoのAppleシリコン上でのネイティブ実行](#expo-goのappleシリコン上でのネイティブ実行)のために必要でした。

これまでと同様に、Expo Goの外部でライブラリを引き続き使用できます。
...
1. [test修正](#test修正)
   - [`react-native-reanimated` v3対応](#react-native-reanimated-v3対応)
...
1. [expo-template-blank-typescriptの更新履歴の確認と対応](#expo-template-blank-typescriptの更新履歴の確認と対応)
1. [expo-template-bare-minimumの更新履歴の確認と対応](#expo-template-bare-minimumの更新履歴の確認と対応)
...
1. [React Native Upgrade Helper](https://react-native-community.github.io/upgrade-helper/?from=0.71.8&to=0.72.3)を参照して、React Nativeの更新を確認
   - このアプリで必要な対応は、[expo-template-bare-minimumの更新](https://github.com/expo/expo/commits/sdk-49/templates/expo-template-bare-minimum)で対応されていました
...
しかし、前回（Expo48への更新時）と同様・`@react-native-firebase`の更新が必要であるため保留としました。
...
```ts title="NativeWebViewComponent"
...
`react-native` v0.72.0から`FlatList`（`VirtualizedList`）の`initialScrollIndex`に`-1`（負の値）を指定するとwarnログがでるようになりました。
...
[expo-template-blank-typescriptの更新履歴](https://github.com/expo/expo/commits/sdk-49/templates/expo-template-blank-typescript)を確認しました。
...
[expo-template-bare-minimumの更新履歴](https://github.com/expo/expo/commits/sdk-49/templates/expo-template-bare-minimum)を確認しました。
...
Android, Xcode関連ファイルは `android/`, `ios/` の `.gitignore` に記載されているため削除されました。
ただし、prebuild時のファイルコピー処理に不具合があり、`prebuild/`フォルダの一部ファイルがignoreできなくなりました。
その対策として、一時的に`prebuild/.gitignore`を追加しています。

```diff title="prebuild/.gitignore"
@@ -0,0 +1,53 @@
+# android/, ios/ ディレクトリの .gitignore が prebuild フォルダにコピーされてないため prebuild/ 直下に一時的に作成している
+# TODO: postprebuild での copy 時に cpx が . 始まりのファイルをコピーしない問題を解消した後、このファイルを削除する
+
+########################
+# android/.gitignore
+# OSX
