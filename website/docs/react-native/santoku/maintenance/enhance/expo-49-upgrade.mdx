...
1. [TSCエラーへの対応](#tscエラーへの対応)
   ...
   - [FunctionComponentの返り値の範囲拡大](#functioncomponentの返り値の範囲拡大)
1. [テストコードの修正](#テストコードの修正)
   - [`react-native-reanimated` `v3`対応](#react-native-reanimated-v3対応)
   - [`react-native` Dimensionsのmock失敗に対応](#react-native-dimensionsのmock失敗に対応)
   - [`jest.advanceTimersByTime`を`act`でwrap](#jestadvancetimersbytimeをactでwrap)
   - [`jest-mock`における`expo-constants`のdeprecated warnに対応](#jest-mockにおけるexpo-constantsのdeprecated-warnに対応)
...
#### `react-native` Dimensionsのmock失敗に対応
...
#### `jest-mock`における`expo-constants`のdeprecated warnに対応
