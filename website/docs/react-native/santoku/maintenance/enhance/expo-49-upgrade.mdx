...
なお、このアプリでは使用していないもの（EAS BuildやEAS Updateなど）に関する内容は記載しません。
:::
...
同期関数はJavaScriptValue、JavaScriptObject、JavaScriptFunctionなどの生のJS型の引数を受け取ることができるようになりました。
...
以前は、追加のschemeを設定するにはconfig pluginを使用する必要がありました。
...
デバッグ ビルドでは明示的に`true`が設定されますが、他のバリアント（例：リリースビルド）では指定されていません。
つまり、API 27以下ではデフォルトで`true`、API 28以降ではデフォルトで`false`になります。

セキュリティで保護されていないエンドポイントへのネットワークリクエストが存在する場合は、expo-build-propertiesを使って`true`にする必要があります。
...
このアプリでは、以下の作業を上から順に実施してExpo SDK 49にアップグレードしました。

- `npm install expo@^49.0.0`を実行して、Expo SDKをアップグレード
...
- [buildエラー修正](#buildエラー修正)
  - [`app.notifee:core`を見つけられない問題対応](#appnotifeecoreを見つけられない問題対応)
- renovate除外設定更新
  - 使用しなくなった`androidx.swiperefreshlayout:swiperefreshlayout`を削除
- [不要になったconfig plugin`withAndroidRemoveUsesClearTextTrafficForRelease`を削除](#不要になったconfig-pluginwithandroidremoveusescleartexttrafficforreleaseを削除)
...
- [React Native Upgrade Helper](https://react-native-community.github.io/upgrade-helper/?from=0.71.8&to=0.72.3)を参照して、React Nativeの更新を確認
...
`npm run android`を実行すると以下のエラーが出ます。
...
### 不要になったconfig plugin`withAndroidRemoveUsesClearTextTrafficForRelease`を削除

今までは暗号化してないhttp通信を禁止するために、`android:usesCleartextTraffic`を削除するconfig pluginを作成し使用していました。
...
そのため、不要となったconfig pluginを削除しました。
...
React Nativeの[CHANGELOG](https://github.com/facebook/react-native/blob/main/CHANGELOG.md#v0723)を参照して、React Nativeの更新内容を確認しました。
