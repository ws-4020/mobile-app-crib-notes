...
`expo-dev-client`パッケージがプロジェクトにインストールされている場合、デフォルトで開発ビルドをターゲットになります。
...
1. `npx expo install --fix`を実行して、Expoが管理するライブラリのアップグレード
   - TypeScript `x5.2.x`ではなく`v5.1.x`を使用するため`npm install typescript@~5.1.3`を追加で実行（[参考](#typescript-バージョン-513)）
