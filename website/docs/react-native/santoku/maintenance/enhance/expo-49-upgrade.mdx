...
このアプリでは、[Expo SDK 48 から 49 にアップグレードする方法](https://blog.expo.dev/expo-sdk-49-c6d398cdf740#a25b)を参考に、以下の作業を上から順に実施してExpo SDK 49にアップグレードしました。
...
- [`npx expo-doctor@latest`を実行してプロジェクトの依存関係に問題がないか確認](#npx-expo-doctorlatestを実行してプロジェクトの依存関係に問題がないか確認)
- [tscチェック](#tscチェック)
...
- [不要になったConfig Plugins`withAndroidRemoveUsesClearTextTrafficForRelease`を削除](#不要になったconfig-pluginswithandroidremoveusescleartexttrafficforreleaseを削除)
