...
1. [テストコードの修正](#テストコードの修正)
   ...
   - [`jest-mock`における`expo-constants`のdeprecated警告に対応](#jest-mockにおけるexpo-constantsのdeprecated警告に対応)
...
#### `jest-mock`における`expo-constants`のdeprecated警告に対応

[Expo SDK 49の主な変更 - Expoがサポートするライブラリや機能 - Expo Constants](#expo-constants)に記載した通り、Constants.manifestが非推奨になりました。
mockする際に[スプレッド構文](https://developer.mozilla.org/ja/docs/Web/JavaScript/Reference/Operators/Spread_syntax)を使用してアクセスすると警告ログが出力されるようになっています。
