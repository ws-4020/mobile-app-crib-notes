...
キャッシュデータが「新しい」ものから「古くなる」までの期間です。[Important Defaults](https://react-query.tanstack.com/guides/important-defaults)で示されるとおり、デフォルト値は 0 です。
デフォルト値のままとし、必要に応じて各クエリ毎に個別で設定することとします。

##### cacheTime

未使用なクエリのキャッシュデータを削除するまでの期間です。[Important Defaults](https://react-query.tanstack.com/guides/important-defaults)で示されるとおり、デフォルト値は 5 分です。
デフォルト値のままとし、必要に応じて各クエリ毎に個別で設定することとします。
