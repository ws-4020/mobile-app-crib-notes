...
これらのオプションは、全てのクエリで有効なデフォルト値を設定することが出来、かつクエリ毎にその値を上書きできます。
ここでは、これらいくつかのオプションについてこのアプリの特性に応じた設定値を検討します。
特に、React Query 公式ドキュメントの[Important Defaults](https://react-query.tanstack.com/guides/important-defaults)で示されるオプションについては、デフォルト値で問題ないかを注意深く検討します。
...
クエリ同様に、ミューテーションにおいても全てのミューテーションで有効なデフォルト値を設定することが出来ます。
