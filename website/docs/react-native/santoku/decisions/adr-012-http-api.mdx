...
これは頻繁なローディングインジケーターの表示を抑え UX 向上に役立ちますが、一方で更新後にも古いデータが見えることによる混乱を引き起こします。
...
この方法は再フェッチが終了するまで一時的に古いデータが表示されるため、混乱を引き起こす可能性があります。

2 つめの方法は、ミューテーションのレスポンスで取得した値でキャッシュデータを更新する方法です。
パフォーマンスに優れた方法ですが、レスポンスに最新のデータが含まれることが前提となり、また実装がやや複雑となります。

このアプリにおいては、更新後に古いデータが見えることなく、かつ出来るだけシンプルな方法で実現したいです。
そこで、ミューテーション成功時に、再フェッチが必要なクエリのキャッシュデータを破棄することで古いデータを表示させない方針とします。実装したコード例は次の通りです。
