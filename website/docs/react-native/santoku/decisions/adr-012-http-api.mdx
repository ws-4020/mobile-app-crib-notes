...
:::note
上記を考慮したデータ取得の方法は、（React公式ドキュメント「[エフェクトを使ったデータフェッチ](https://ja.react.dev/reference/react/useEffect#fetching-data-with-effects)」で紹介されている）[こちらの記事](https://maxrozen.com/race-conditions-fetching-data-react-with-useeffect)をご覧ください。
:::
