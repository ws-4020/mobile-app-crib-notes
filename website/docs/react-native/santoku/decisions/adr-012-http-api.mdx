...
これにより、SWRに近いシンプルな記述を実現することが出来ます。
このアプリではOpenAPI仕様からソースコードを自動生成することにより、REST APIとの整合性や開発効率性の向上を図ります。
その為、このオプションは利用しません。

##### retry

クエリ失敗時のリトライ回数です。
[Important Defaults](https://react-query.tanstack.com/guides/important-defaults)で示されるとおり、デフォルト値は3です。
このアプリでは`false`をデフォルト値としてリトライせず、リトライが必要なクエリについては各クエリ毎に個別で設定することとします。
...
キャッシュデータが「新しい」ものから「古くなる」までの期間です。
[Important Defaults](https://react-query.tanstack.com/guides/important-defaults)で示されるとおり、デフォルト値は0です。
デフォルト値のままとし、必要に応じて各クエリ毎に個別で設定することとします。

##### cacheTime

未使用なクエリのキャッシュデータを削除するまでの期間です。
[Important Defaults](https://react-query.tanstack.com/guides/important-defaults)で示されるとおり、デフォルト値は5分です。
デフォルト値のままとし、必要に応じて各クエリ毎に個別で設定することとします。
