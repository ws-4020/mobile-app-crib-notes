...
- Fetch APIとReduxを用いる案はいくつかの理由により採用しません。
...
- ローディング状態時は画面が真っ白となる
- サーバへのリクエストがエラーになった際に、画面表示への反映やリトライ処理などが行われない
...
- HTTP API通信にはFetch APIなどを利用し、その非同期状態や取得したバックエンドデータはReduxなどの状態を管理するライブラリを用いて管理
- データフェッチ用ライブラリの導入
...
- `staleTime`：クエリが「新しい」ものから「古くなる」までの期間。クエリが「新しい」限り、クエリは取得済みのデータを返すため、ネットワークリクエストは発生しません。クエリが「古い」場合（デフォルトはフェッチ後すぐ古くなる）、クエリは取得済みのデータを返し、特定の条件下にてバックグラウンドで再フェッチします。
