...
:::note
Dynamic Linksを利用すると簡単に組み込めますが、注意する点があります。（セキュリティの課題やReact Nativeアプリの場合[iOSのissue](https://github.com/invertase/react-native-firebase/issues/2660)があります）
...
本プロジェクトではWebサイトで機能を提供しないため、Firebaseが用意しているWebサイトを利用できるDynamic Linksのこの機能は有効です。

### セキュリティ

カスタムURLスキームとは異なり、アプリケーションとURL（ドメイン）の関連付けが正当であることをOSが検証しているため、他のアプリに情報が意図せず共有されることはありません。

認証、認可が必要な場合、情報自体はURLに含めず、取得するためのパラメータや一時的なトークンのみをURLに含め、保護したい情報はAPIから取得します。
...
どちらかのAPIを利用すれば実装できますが、下記の点を考慮しCold Startする場合はPlatformを判定して呼び分けます。

 - React Native Firebaseには[iOSのissue](https://github.com/invertase/react-native-firebase/issues/2660)があるためLinking API
 - React Native Linkingには[Androidのissue](https://github.com/facebook/react-native/issues/25675)があるためReact Native Firebase API
