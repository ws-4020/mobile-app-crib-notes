...
一方で、[iOSのペーストボードの使用を無効に](https://firebase.google.com/docs/dynamic-links/ios/receive?hl=ja#set-up-firebase-and-the-dynamic-links-sdk)しても、初回起動時に読み取ることができます。

:::note
[公式ドキュメント](https://firebase.google.com/docs/dynamic-links/operating-system-integrations)では、iOSの場合クリップボードを利用するという記載があります。
...
クリップボードやOS機能でディープリンクURLを取得した場合は間違いなくユーザが意図したURLと言えますが、それ以外の方法で取得したURLはユーザとは無関係のURLの可能性があります。
