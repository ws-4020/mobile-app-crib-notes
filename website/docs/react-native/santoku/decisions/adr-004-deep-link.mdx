...
Dynamic Linksを利用する場合は、どちらかのAPIを利用すれば実装できます。ただし、下記の点を考慮しアプリ起動時にはPlatformを判定して呼び分けます。
 ...
 - React Native Linkingには[AndroidでディーリンクのURLを読み取れないというissue](https://github.com/facebook/react-native/issues/25675)があるためReact Native Firebaseを利用してコンテンツリンクURLを取得します。
