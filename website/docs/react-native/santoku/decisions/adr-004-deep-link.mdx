...
Firebase SDKの[実装](https://github.com/firebase/firebase-ios-sdk/blob/8.7.0/FirebaseDynamicLinks/Sources/FIRDynamicLinkNetworking.m#L190)からFirebaseがデバイスの情報などを利用してサーバに保存していると想定されます。

実際に次のようにアプリを起動すると、クリップボードに値がない場合でも、ディープリンクのURLが読み取れます。
 ...
 - `テキスト`のような文字列をクリップボードにコピーする（OpenボタンをタップしたときにディープリンクURLがクリップボードにコピーされているので上書きする）
 - アプリをインストールして起動する
