...
|Dynamic Links|App Links、Universal Linksを拡張したFirebaseの機能|カスタムURLは対象外|
...
そのため、アプリケーションをインストールしていないユーザ向けに、導線となるウェブサイトも用意する必要があります。

Dynamic Linksでは、未インストールのユーザに対する導線が簡単に用意できるので、ウェブサイトを持たないサービスでも簡単にディープリンクを導入できます。
...
Dynamic Linksを利用する場合は、どちらかのAPIを利用すれば実装できます。ただし、下記の点を考慮しアプリ起動時にはPlatformを判定して呼び分けます。
 ...
 - React Native Firebaseには[iOSで起動時間が短いと読み取れないissue](https://github.com/invertase/react-native-firebase/issues/2660)があるためReact Native Linking APIを利用してディープリンクURLを取得します。
   - ディープリンクURLに含まれるコンテンツリンクURLを取得するために[resolveLink](https://rnfirebase.io/reference/dynamic-links#resolveLink)を利用します。
 - React Native Linkingには[AndroidはLikingで読み取れないissue](https://github.com/facebook/react-native/issues/25675)があるためReact Native Firebase APIを利用してコンテンツリンクURLを取得します。
