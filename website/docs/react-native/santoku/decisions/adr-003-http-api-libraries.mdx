...
SantokuAppでは、HTTP API通信に以下のライブラリを使用します。
...
React Nativeを使用したモバイルアプリでは、HTTP APIクライアントとして[Fetch API](https://developer.mozilla.org/ja/docs/Web/API/Fetch_API)が使用できます。また、[Frisbee](https://github.com/niftylettuce/frisbee)、[axios](https://github.com/axios/axios)などの[XMLHttpRequest API](https://developer.mozilla.org/ja/docs/Web/API/XMLHttpRequest)に依存したライブラリも使用可能です。
...
React Queryを使用しない場合、エラーのハンドリングやローディング中などの状態を管理するのが大きな課題となります。React Queryを使用するとそれらの管理が非常に楽になり、効率良く開発ができます。また、HTTPリクエストのキャッシュ管理やページネーション、無限スクロールの対応など多くの便利な機能が搭載されています。
...
そういった懸念はありますが、React Queryを使用した場合の方式設計や開発ガイドを整備することにより、効率的に開発を進めることができると考えています。よって、SantokuAppではReact Queryを採用することにします。

## 決定

- HTTP APIクライアントは、axiosを使用する
