...
ユーザが`ScreenB`に遷移するディープリンクをタップすると、`DeepLinkStackNavigator`と`ScreenB`がスタックに追加されます。
...
この場合、`ScreenB`から`navigation.goBack`を実行すると、`ScreenC`に戻る事が可能です。
</details>
...
この場合、`ScreenB`から`navigation.goBack`を実行すると、`ScreenF`に戻る事が可能です。
</details>
