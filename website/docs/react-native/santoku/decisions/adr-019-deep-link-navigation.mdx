...
未認証の場合は、URLと画面のマッピングが定義されていないため、特に何も起こりません。コールド、ウォームスタートの場合は、未認証時の初期画面が表示され、ホットスタートの場合はユーザが開いている画面がそのまま表示されます。
...
詳細は、[React Navigationのlinkingの調査](./linking-feasibility)を参照してください。
  ...
  useEffect(() => {
    const subscription = Linking.addEventListener('url', event => {
      ...
      if (parsedURL.path?.startsWith('screen-a')) {
        navigation.dispatch(StackActions.push('StackNavigatorA', {screen: 'ScreenA'}));
      } else if (parsedURL.path?.startsWith('screen-b')) {
        navigation.navigate('StackNavigatorA', {screen: 'ScreenB'});
