...
併せて`expo eject`は非推奨になり、新たに[`npx expo prebuild`](https://docs.expo.dev/workflow/prebuild/)と[Config Plugins](https://docs.expo.dev/config-plugins/introduction/)が提供されました。
...
| Config Plugins | ネイティブプロジェクト内のリソースをカスタマイズする機能です。Expoが管理するテンプレートから変更したい箇所を自由に変更できます。                                                                                                                                             |

Expo EASの内部でも使われており、この仕組みを利用して以前では実現できなかったネイティブコードの自由な記述が可能となっています。
