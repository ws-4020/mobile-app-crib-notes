...
Managed workflowは、自身で管理するのは基本的にJavaScriptやAssetのみになります。ネイティブコードなどはExpoが管理するため、アプリの初期プロジェクトは以下のような構成になります。
...
しかし、Expo SDK 41からExpo EASが使用できるようになり、上記の制約がなくなりました。現在では、Managed workflowでもネイティブコードを自由に記述できるようになり、アプリのビルドもローカルPCで実施できます。

併せて`expo eject`は非推奨になり、新たに[`npx expo prebuild`](https://docs.expo.dev/workflow/prebuild/)が提供されました。

Prebuildは、アプリの依存ライブラリや設定ファイル（`app.json`）に基づいて、ネイティブプロジェクトを自動生成する機能です。Expo EASの内部でも使われており、この仕組みを利用して以前では実現できなかったネイティブコードの自由な記述が可能となっています。

Managed workflowでありながら、Prebuildを組み合わせることで、Bare workflowのようにアプリを開発できます。この学習コンテンツでも、Managed workflowとPrebuildを組み合わせてアプリを開発します。
