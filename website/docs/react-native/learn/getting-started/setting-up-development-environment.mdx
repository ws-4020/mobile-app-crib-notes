...
| macOS, Windows | Git                  | 最新                                                  | Homebrew(iOS), [公式ページ](https://git-scm.com/download/win)(Windows)でインストール |
...
:::note info
Java11を利用する場合はcmdline-toolsのlatestではなく10.0を利用してください。
設定方法は下記の手順で実施してください。
1. Android Studio > SDK Manager > SDK Tools > 「Show Package Details」にチェック > Android SDK Command-line Tools(latest) > Version 10.0にチェックを付けて「Apply」実施（cmdline-toolsがインストールされる）
2. 環境変数は**``$ANDROID_SDK_ROOT/cmdline-tools/latest/bin``**から**`$ANDROID_SDK_ROOT/cmdline-tools/10.0/bin`**に修正
:::
...
macOSでの`~/.zprofile`への設定例は次のようになります。
...
* macOSでシェルに応じて環境設定ファイルを設定する方法
  * ログインシェルがBashの場合
    * 環境変数の設定は`~/.profile`ファイルで行います。
    * 環境設定は`.bashrc`ファイルで行います。（'.bashrc'ファイルが存在しない場合、`touch ~/.bashrc`で生成できます。）
  * ログインシェルがZshの場合
    * 環境変数の設定は`~/.zprofile`ファイルで行います。
    * 環境設定は`.zprofile`ファイルで行います。（'.zprofile'ファイルが存在しない場合、`touch ~/.zprofile`で生成できる）

※ `ln -s ~/.profile ~/.zprofile`コマンドを使用して、BashとZsh両方同じ設定を共有することもできます。
:::
...
インストール及び使い方については、[asdfのインストール](/react-native/learn/getting-started/setting-up-development-environment#asdfのインストール)を参考にしてください。

:::danger warning
Appleシリコン搭載のMacコンピュータでは、Rubyのバージョンが2.7.x未満の場合にCocoaPodsのインストールでエラーが発生した場合、下記を実行することでCocoaPodsをインストールできます。
```bash
...
上記の方法でも失敗した場合はHomebrewやasdf-vmなどのツールを利用して、**使用するRubyのバージョンを2.7.x以上にアップデートしてください。**

Rubyのバージョンによって、Appleシリコン搭載のMacでCocoaPodsがインストールできない場合があります。
Appleシリコン搭載のMacはARMアーキテクチャを採用しており、Rubyの一部のバージョンはARMに適切に対応していないため、CocoaPodsのインストールに問題が生じる可能性があります。
この問題を解決する方法はx86_64アーキテクチャ向けにRubyをインストールし、それを使用してCocoaPodsをインストールすることです。
また、Rosetta2を使用してARMバージョンのRubyでCocoaPodsを実行する方法もありますが、一部のバージョンでは問題が発生することがあります。
:::
